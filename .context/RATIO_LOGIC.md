...
| **ROIC %** | `NOPAT / Average Invested Capital` | **NOPAT**: `(Operating Profit - Depreciation) * (1 - Tax Rate)` <br> **Tax Rate**: Effective tax rate from P&L (`Tax %`) <br> **Invested Capital**: `(Equity + Reserves + Borrowings) - Cash Equivalents - Investments` <br> **Cash Equivalents**: Balance Sheet (`Cash Equivalents`) <br> **Investments**: Balance Sheet (`Investments`, fetched via `DeepFetcher`). |
...
| **Current Ratio** | `Current Assets / Current Liabilities` | **Current Assets**: `Inventories + Trade receivables + Cash Equivalents + Loans n Advances + Other asset items` <br> **Current Liabilities**: `Trade Payables + Advance from Customers + Other liability items` <br> *Note: Uses granular data fetched via API. Ensure "Other asset items" is used instead of the parent "Other Assets".* |
...
| **CFO / EBITDA** | `CFO / EBITDA` | **CFO**: Cash Flow table (`Cash from Operating Activity`) <br> **EBITDA**: `Operating Profit` (Preferred, as it is pre-depreciation core EBITDA) OR `Operating Profit + Other Income`. |
...
| **Free Cash Flow** | `CFO - Net Capex` | **CFO**: Cash Flow (`Cash from Operating Activity`) <br> **Net Capex**: `Fixed assets purchased` (absolute outflow) - `Fixed assets sold` (inflow) <br> *Note: Both Capex items are silently fetched via API from `Cash from Investing Activity` breakdown.* |
...
2.  **`Other Liabilities` (Balance Sheet)**: Extracts `Trade Payables`, `Advance from Customers`, `Other liability items`.
